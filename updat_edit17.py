# -*- coding: utf-8 -*-
"""
Created on Sat Aug 24 08:53:24 2019
...
n=int(n)
tst=[0][0]
tst=np.array(tst, dtype=np.float32)
#print(n)
i=0
while i<n:   
    #tst[0].append(lst[0][(i*127+i):((i+1)*127+i)])
    print(tst.shape)
    vecs.append(tst)
    i=i+1
...
while(cap.isOpened()):
    ...
    if z%4==0 and ret==True:    
        ...
        for face in faces:
            ...
            #cont=file.write()
            n=0
            with open('pl.csv', 'a') as csvFile:
                while n<128:
                    emv=(face['embedding'][0][n])
                    csvData = [[m,emv]]
                #print(emv)
                ...
                #lst.append(emv)
                #dict={"index":m,"embed":lst}
                #df=pd.DataFrame(dict)
                #df.to_csv('fota.py',index=False)
                    n=n+1
            #fields=['first','second','third']
           # with open('fota.csv', 'a') as f:
            #    writer = csv.writer(f)
...
#print(vecs[0])
#while(n<=m):
#dict={"index":0,"embed":lst}
#df=pd.DataFrame(dict)
#df.to_csv('fota.csv',index=False)
    #n=n+1
